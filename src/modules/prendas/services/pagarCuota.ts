...
export const pagarVentaCompleta = async (id_venta: string) => {
  const client = await db.connect();
  try {
    await client.query('BEGIN');
    ...
    await client.query(
      'UPDATE ventas SET id_estado_pago = 2 WHERE id_venta = $1',
      [id_venta]
    );


    await client.query(
      'UPDATE venta_cuotas SET id_estado_pago = 2 WHERE id_venta = $1',
      [id_venta]
    );

    await client.query('COMMIT');
    return { success: true, nuevoEstadoVenta: 2 };
  } catch (error) {
    await client.query('ROLLBACK');
    throw error;
  } finally {
